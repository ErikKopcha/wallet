...
const MobileDashTable = () => {
  ...
  const editedTransactions = sortedTransactions.map(transaction =>
    transactionRefactor(transaction, categories),
  );
  ...
  return (
    <>
      {status === 'loading' ? (
        <Loader left={'45%'} top={'500%'} zIndex={5} />
            ...
            >
              <CardContent sx={{ padding: 0 }}>
                {columns.map(column => (
                  ...
                  >
                    <Typography key={uniqid()}>{column.id}</Typography>
                    {transaction[column.id].length >= 30 ? (
                      <Tooltip key={uniqid()} title={transaction[column.id]}>
                        <Typography
                          key={uniqid()}
                          sx={{
                            maxWidth: '49%',
                      ...
                      <Typography key={uniqid()} sx={{ maxWidth: '200px' }}>
                        {transaction[column.id]}
                      </Typography>
                    )}
                  </Box>
